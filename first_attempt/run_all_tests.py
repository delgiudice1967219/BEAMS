"""
Unified test script to run B-cos, GradCAM, and CLIP-ES on test images.
Includes testing with negative categories (not present in the image).
"""
...
from clip_es_baseline import compute_clip_es_heatmap
...
def run_tests():
    ...
    # 3. Run Tests
    output_dir = "test_results_all_methods"
    ...
    for case in test_cases:
        ...
        # Rows: Prompts
        # Cols: Original, B-cos, GradCAM, CLIP-ES
        n_rows = len(prompts)
        n_cols = 4
        ...
        for i, prompt in enumerate(prompts):
            ...
            # Column 3: CLIP-ES
            print(f"  CLIP-ES: {prompt}")
            heatmap_clipes, score_clipes = compute_clip_es_heatmap(
                image, prompt, plain_clip_model, plain_preprocess, plain_device, return_raw=True
            )
            axes[i, 3].imshow(image)
            axes[i, 3].imshow(heatmap_clipes, cmap='jet', alpha=0.5)
            axes[i, 3].set_title(f"CLIP-ES\nScore: {score_clipes:.4f}")
            axes[i, 3].axis('off')
