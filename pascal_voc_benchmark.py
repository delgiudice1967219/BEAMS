...
#         print(f"\n==========================================")
#         print(f"Final mIoU (Improved Pipeline): {mean_iou:.2f}%")
#         print(f"==========================================")
#     else:
...
def run_benchmark():
    ...
    ious = []
    # Dictionary to store IoUs for each class separately
    class_ious = {name: [] for name in VOC_CLASSES if name != "background"}
    ...
    for i, data in enumerate(tqdm(loader, total=max_images)):
        ...
        try:
            ...
            # Add IoU to the specific class list
            if label_name in class_ious:
                class_ious[label_name].append(iou)
    ...
    print("==========================================")
    # Print individual class scores
    for cls_name in VOC_CLASSES:
        if cls_name == "background":
            continue

        scores = class_ious.get(cls_name, [])
        if len(scores) > 0:
            cls_mean = np.mean(scores) * 100
            print(f"{cls_name:<15}: {cls_mean:.2f}%")
        else:
            print(f"{cls_name:<15}: N/A (No samples)")
