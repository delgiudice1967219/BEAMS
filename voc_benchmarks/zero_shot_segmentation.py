...
def apply_dense_crf_on_hard_mask(original_image_rgb, hard_mask):
    """
    Refines a binary HARD MASK using DenseCRF to snap to image edges.
    This is much more robust than refining the raw heatmap.
    """
    if not HAS_CRF:
    ...
    # Ensure hard_mask is same size and binary (0 or 1 float32)
    ...
    # --- UNARY POTENTIAL DALLA HARD MASK ---
    # La hard mask è la nostra "certezza".
    # Dove è 1 (Oggetto), diamo alta probabilità (es. 0.95).
    # Dove è 0 (Sfondo), diamo bassa probabilità (es. 0.05).
    # Questo "ancora" il CRF alla tua maschera buona.

    # Probabilità dell'oggetto
    prob_object = hard_mask_resized * 0.9 + 0.05  # 0 -> 0.05, 1 -> 0.95

    # U = -log(P)
    # Background Prob = 1 - Object Prob
    ...
    # --- PARAMETRI (Manteniamo quelli "stretti" che ti ho dato prima) ---
    # Smoothness spaziale locale
    ...
    d.addPairwiseBilateral(
        sxy=(50, 50),  # Medio raggio per il colore
        srgb=(13, 13, 13),
    ...
    Q = d.inference(5)  # 5 iterazioni sono sufficienti
...
def run_segmentation_emo(
    ...
    # 2. Refinement (DenseCRF)
    print("Applying DenseCRF...")
    # Resize raw map to original size for visualization consistency before CRF
    raw_map_resized = cv2.resize(
        raw_map, (image.width, image.height), interpolation=cv2.INTER_LINEAR
    )

    # Apply CRF (returns binary-like float mask)
    crf_mask = apply_dense_crf(original_image_np, raw_map)

    # 3. Hard Mask Generation (from Raw Map, or CRF result?)
    # The prompt says: "Implement a function get_hard_mask(refined_heatmap, ...)"
    # And "Refinement Stage: DenseCRF ... Output: A binary segmentation mask."
    # So CRF already gives a binary mask.
    # But "Final Binary Mask Generation ... The raw heatmap is continuous... I need a crisp binary mask".
    # It seems "get_hard_mask" is intended for the continuous heatmap (e.g. if CRF is skipped or as an alternative).
    # Or maybe "refined_heatmap" in get_hard_mask refers to the B-cos map after background suppression?
    # Let's apply get_hard_mask to the B-cos map (after BG suppression) as a baseline,
    # AND use the CRF output as the "Final CRF Mask".
    # The user asks to visualize: "Refined Map (Parts + Background Suppression)", "Final CRF Mask", "final SAM like MAsk (Hard Mask)".
    # "final SAM like MAsk" implies the high quality one.
    # If CRF is available, CRF mask IS the hard mask.
    # If not, we use get_hard_mask on the heatmap.
    # Let's generate both for comparison.
    ...
    plt.title("Hard Mask (Thresholding)")
    plt.axis("off")

    # CRF Mask
    plt.subplot(1, 5, 4)
    plt.imshow(crf_mask, cmap="gray")
    plt.title("DenseCRF Mask")
    plt.axis("off")
    ...
    # Draw contours on the plot is tricky with imshow, better to draw on image
    # But we can plot the contour coordinates
