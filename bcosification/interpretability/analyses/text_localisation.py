...
def save_visualizations(fig, axs, row, original_img, class_name, grad_image, contribs, vrange, caption, score, use_class_name=False):
    cmap = plt.cm.bwr
    ...
    if use_class_name:
        ...
        axs[1].axis('off')
        axs[1].set_title(f"B-cos explanations: {caption}\nScore: {score:.4f}", fontsize=12)
        ...
        axs[row, 1].axis('off')
        axs[row, 1].set_title(f"B-cos explanations: {caption}\nScore: {score:.4f}", fontsize=12)
...
def main():
    ...
    for i, test_text_name in enumerate(text_to_localize_list):
        ...
        grad_image, contribs, vrange, score = compute_attributions(model, test_img, zeroshot_weight, args.smooth, 
                                                            pool_cosine=args.pool_cosine, norm_max_cosine=args.norm_max_cosine)
        if args.use_class_name or len(text_to_localize_list) == 1:
            im = save_visualizations(fig, axs, i, original_img, class_name, grad_image, contribs, vrange, test_text_name, score, True)
        else:
            im = save_visualizations(fig, axs, i, original_img, class_name, grad_image, contribs, vrange, test_text_name, score, False)
